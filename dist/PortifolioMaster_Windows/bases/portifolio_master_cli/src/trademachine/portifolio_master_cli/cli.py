...
from trademachine.portifoliomaster.services.cache import CacheService
...
class PortifolioCLI:
    ...
    def _optimization_output_service(self) -> OptimizationOutputService:
        ...
        return OptimizationOutputService(
            ...
            report_generator=generate_portfolio_report_html,
        )
    ...
    def load_reports(
        ...
        cache_service = self._cache_service(cache_filepath)
        self._correlation_cache.clear()
        ...
        configured_workers = (
            ...
            else self.default_config.get("load_workers", 0)
        )
    ...
    def _handle_optimization_results(
        ...
        if kwargs.get("plot_chart"):
            report_path = self._optimization_output_service().generate_report(
    ...
    def _needs_detailed_metrics(self, kwargs: dict) -> bool:
    ...
    def _print_benchmark_result(self, result: BenchmarkResult) -> None:
        ...
        print(
            f"Processed in sample:    {result.processed_combinations:,} combinations"
        )
        print(
    ...
    def _suffix_output_path(self, path: str, suffix: str) -> str:
    ...
    def _run_greedy_loops(self, **kwargs) -> bool:
        ...
        for loop_idx in range(1, requested_loops + 1):
            ...
            if kwargs.get("save_trades_prefix"):
                loop_kwargs["save_trades_prefix"] = self._suffix_output_path(
            ...
            removed = self._remove_selected_strategies(combo)
        ...
        logger.info(f"[output] multi_greedy_summary.json → {os.path.abspath(summary_path)}")
    ...
    def run_optimization(self, **kwargs) -> bool:
        ...
        try:
            ...
            if kwargs.get("remove_top1_from_cache") and engine.best_portfolios:
                self._remove_selected_strategies(
    ...
    def _apply_strategy_exclusion(
        ...
        missing = [n for n in excluded_names if n not in self.loaded_expert_names]
        if missing:
            logger.warning(
        ...
        remaining_names = [
            strategy_name
            for strategy_name in self.loaded_expert_names
    ...
    def cache_merge(self, left: str, right: str, destination: str) -> None:
        ...
        if len({os.path.abspath(left), os.path.abspath(right), os.path.abspath(destination)}) < 3:
            raise ValidationError(
    ...
    def drawdown_pairing(
        ...
        apply_result = pairing_service.apply(self.portfolio_manager, rows)
        ...
        if export_passed_reports:
            ...
            for s in result.strategies:
                ...
                if os.path.abspath(destination) == os.path.abspath(s.mt5_report_path):
                    destination = os.path.join(
                        passed_reports_dir, f"{s.name}_{os.path.basename(s.mt5_report_path)}"
                    )
