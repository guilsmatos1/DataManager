...
def parse_sqx_csv(filepath: str) -> tuple[str, np.ndarray, list[str]]:
    ...
    # Enforce deterministic adherence input order: ascending Ticket.
    # Use stable sorting so duplicate tickets preserve original relative order.
    tickets = pd.to_numeric(df.iloc[:, _COL_TICKET], errors="coerce")
    df = df.assign(_ticket_sort=tickets).sort_values(
        by="_ticket_sort", kind="mergesort", na_position="last"
    )

    # Use positional access for duplicate-named columns
    profit_loss = pd.to_numeric(df.iloc[:, _COL_PROFIT_LOSS], errors="coerce").fillna(
        0.0
    )
...
def parse_sqx_directory(
    directory: str, show_progress: bool = False
