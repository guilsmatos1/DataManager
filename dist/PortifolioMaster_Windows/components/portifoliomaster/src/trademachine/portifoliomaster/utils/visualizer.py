...
def _correlation_cell_style(value: float, diagonal: bool = False) -> str:
    """Returns inline CSS for a correlation heatmap cell."""
    if diagonal:
        return (
            "background:#10324d;color:#4fc3f7;font-weight:700;"
            "border:1px solid #27557d;"
        )
    if np.isnan(value):
        return "background:#1f2740;color:#7f8ca6;border:1px solid #2d3756;"

    strength = abs(float(value))
    if strength < 0.20:
        return "background:#b7ff00;color:#173100;border:1px solid #8fc500;"
    if strength < 0.35:
        return "background:#8ccf00;color:#152600;border:1px solid #679900;"
    if strength < 0.50:
        return "background:#caa100;color:#261900;border:1px solid #997600;"
    return "background:#e74c3c;color:#fff5f3;border:1px solid #b93a2d;"


def _build_correlation_section_html(
    combo: tuple[str, ...],
    strategy_names: list[str] | None,
    correlation_matrix: np.ndarray | None,
    correlation_period: str,
) -> str:
    """Builds a heatmap-like HTML correlation table for a portfolio combo."""
    if not combo:
        return (
            '<div class="correlation-panel"><p class="no-corr">'
            "Correlation matrix unavailable for an empty portfolio."
            "</p></div>"
        )

    if correlation_matrix is None or not strategy_names:
        return (
            '<div class="correlation-panel"><p class="no-corr">'
            "Correlation matrix unavailable for this report."
            "</p></div>"
        )

    name_to_idx = {name: idx for idx, name in enumerate(strategy_names)}
    present_names = [name for name in combo if name in name_to_idx]
    if len(present_names) != len(combo):
        return (
            '<div class="correlation-panel"><p class="no-corr">'
            "Correlation matrix could not be aligned with the selected portfolio."
            "</p></div>"
        )

    indices = [name_to_idx[name] for name in present_names]
    submatrix = correlation_matrix[np.ix_(indices, indices)]

    valid_pairs: list[tuple[str, str, float]] = []
    for i in range(len(present_names)):
        for j in range(i + 1, len(present_names)):
            value = float(submatrix[i, j])
            if np.isnan(value):
                continue
            valid_pairs.append((present_names[i], present_names[j], value))

    if valid_pairs:
        max_pair = max(value for _, _, value in valid_pairs)
        mean_abs = float(np.mean([abs(value) for _, _, value in valid_pairs]))
        worst_left, worst_right, worst_value = max(valid_pairs, key=lambda item: item[2])
        max_pair_label = f"{max_pair:.2f}"
        mean_abs_label = f"{mean_abs:.2f}"
        worst_pair_label = (
            f"{escape(worst_left)} x {escape(worst_right)} ({worst_value:.2f})"
        )
    else:
        max_pair_label = "N/A"
        mean_abs_label = "N/A"
        worst_pair_label = "No valid pair overlap"

    header_cells = "".join(
        f"<th>{escape(name)}</th>" for name in present_names
    )
    body_rows = []
    for row_idx, row_name in enumerate(present_names):
        row_cells = []
        for col_idx, value in enumerate(submatrix[row_idx]):
            diagonal = row_idx == col_idx
            label = "N/A" if np.isnan(value) else f"{float(value):.2f}"
            row_cells.append(
                '<td class="corr-cell" style="'
                f"{_correlation_cell_style(float(value), diagonal=diagonal)}"
                f'">{label}</td>'
            )
        body_rows.append(
            f"<tr><th>{escape(row_name)}</th>{''.join(row_cells)}</tr>"
        )

    return (
        '<div class="correlation-panel">'
        '<div class="correlation-header">'
        '<div class="correlation-title">Correlation Matrix</div>'
        f'<div class="correlation-period">Period used in optimization: {escape(correlation_period.upper())}</div>'
        "</div>"
        '<div class="correlation-summary">'
        '<div class="summary-card">'
        '<div class="summary-label">Max Pair Corr</div>'
        f'<div class="summary-value">{max_pair_label}</div>'
        "</div>"
        '<div class="summary-card">'
        '<div class="summary-label">Mean |Corr|</div>'
        f'<div class="summary-value">{mean_abs_label}</div>'
        "</div>"
        '<div class="summary-card">'
        '<div class="summary-label">Worst Pair</div>'
        f'<div class="summary-value">{worst_pair_label}</div>'
        "</div>"
        "</div>"
        '<div class="corr-table-wrapper"><table class="corr-table">'
...
def generate_portfolio_report_html(
    ...
    chart_sections_html = "\n".join(
        ...
        f'<span class="chart-title">Portfolio #{i + 1} &mdash; {escape(", ".join(p.get("Combo", ())))}</span>'
        f"</div>"
        f'<div class="chart-body">'
        f'<div id="chart-plot-{i}" class="chart-plot"></div>'
        f"{_build_correlation_section_html(tuple(p.get('Combo', ())), strategy_names, correlation_matrix, correlation_period)}"
        f"</div>"
        f"</div>"
...
.chart-body {{ flex: 1; overflow: auto; padding: 12px; min-height: 0; display: flex; flex-direction: column; gap: 16px; }}
.chart-plot {{
  min-height: 580px;
  background: #121b30;
...
.correlation-title {{ color: #4fc3f7; font-size: 1rem; font-weight: 700; }}
.correlation-period {{ color: #9aa0b4; font-size: 0.84rem; }}
.correlation-summary {{
  display: grid;
...
.summary-value {{ color: #f5f7ff; font-size: 1rem; font-weight: 700; margin-top: 6px; }}
.corr-table-wrapper {{ overflow: auto; }}
.corr-table {{ width: max-content; min-width: 100%; border-collapse: separate; border-spacing: 6px; }}
.corr-table th {{
  background: #1e1e38;
  color: #cfd8ea;
  padding: 8px 10px;
...
<div id="table-view">
  <div class="report-header">
    ...
    <div class="subtitle">Generated {generated_at} &mdash; {n} portfolio(s) ranked &mdash; click any row to view equity chart and correlation matrix</div>
  </div>
...
function showChart(idx) {{
  ...
  if (!initializedCharts[idx]) {{
    ...
    var body = document.getElementById('chart-plot-' + idx);
    ...
    requestAnimationFrame(function() {{
      var plotDiv = document.getElementById('chart-plot-' + idx).querySelector('.js-plotly-plot');
...
def generate_adherence_report_html(
    ...
    def _as_iso_datetime(ts: str) -> str:
        ...
        for fmt in (
            ...
            try:
                return datetime.strptime(normalized, fmt).isoformat(sep=" ")
            except ValueError:
                continue
        return normalized
    ...
    def _build_time_series(
        ...
        start_ts = f"{shared_start} 00:00:00" if shared_start else None
        end_ts = f"{shared_end} 23:59:59" if shared_end else None

        if start_ts:
            x_vals.append(_as_iso_datetime(start_ts))
            y_vals.append(0.0)

        for ts, eq in zip(timestamps, equity[1:], strict=False):
            x_vals.append(_as_iso_datetime(str(ts)))
            y_vals.append(round(float(eq), 2))

        final_equity = round(float(equity[-1]), 2) if equity else 0.0
        if end_ts:
            end_iso = _as_iso_datetime(end_ts)
            if not x_vals or x_vals[-1] != end_iso:
                x_vals.append(end_iso)
                y_vals.append(final_equity)

        if not x_vals:
            x_vals = [_as_iso_datetime(start_ts or end_ts or "1970-01-01 00:00:00")]
            y_vals = [0.0]
    ...
    for s in result.strategies:
        mt5_x, mt5_y = _build_time_series(
            s.mt5_timestamps.tolist() if hasattr(s.mt5_timestamps, "tolist") else s.mt5_timestamps,
            [round(float(v), 2) for v in s.mt5_equity],
            s.shared_history_start,
            s.shared_history_end,
        )
        sqx_x, sqx_y = _build_time_series(
            s.sqx_timestamps.tolist() if hasattr(s.sqx_timestamps, "tolist") else s.sqx_timestamps,
            [round(float(v), 2) for v in s.sqx_equity],
            s.shared_history_start,
            s.shared_history_end,
        )
        ...
        fig.add_trace(
            go.Scatter(
                ...
                hovertemplate="Date %{x}<br>Equity: %{y:,.2f}<extra>MT5</extra>",
            )
        )
        fig.add_trace(
            go.Scatter(
                ...
                hovertemplate="Date %{x}<br>Equity: %{y:,.2f}<extra>SQX</extra>",
            )
        )
        fig.update_layout(
            template="plotly_dark",
            ...
            xaxis=dict(title="Shared History Date", gridcolor="#252545", type="date"),
    ...
    for i, s in enumerate(result.strategies):
        if s.insufficient_data:
            ...
            status_color = "#f39c12"
            row_bg = "background:rgba(243,156,18,0.08);"
        else:
        ...
        <tr id="chart-row-{i}" style="display:none">
          <td colspan="10" style="padding:0">
            <div style="padding:12px 14px 0;color:#9aa4b2;font-size:12px">
              Shared window used in calculations: <strong>{s.shared_history_start or "N/A"} → {s.shared_history_end or "N/A"}</strong>
            </div>
...
<div class="container">
  <p class="threshold-note">
    ...
    Equity curves and all metrics below use only the shared MT5/SQX history window. Click a row to compare equity curves.
  </p>
  <table>
    <thead>
      <tr>
