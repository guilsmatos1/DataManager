...
class PortfolioManager:
    ...
    def round_lot(value: float) -> float:
        """Normalizes lot values to 2 decimal places."""
    ...
    def save_lots_cache(self, cache_path: str) -> None:
        ...
        try:
            data: dict = {
                name: self.round_lot(lot) for name, lot in self.strategy_lots.items()
            }
            if self._lots_meta:
    ...
    def load_lots_cache(self, cache_path: str) -> None:
        ...
        try:
            ...
            self.strategy_lots = {
                name: self.round_lot(lot) for name, lot in raw.items()
            }
            ...
            try:
                ...
                if len(positive):
                    ...
                    if median_val is not None:
                        self.strategy_lots[name] = self.round_lot(median_val)  # type: ignore[arg-type]
            except Exception:
                logger.debug(
    ...
    def get_returns_matrix(self) -> pl.DataFrame:
        ...
        all_strategy_dfs = [
            df.select(["Horário", "Net_Profit"]).with_columns(
                pl.lit(name).alias("strat_name")
            )
            for name, df in self.strategies.items()
        ]
