...
MT5_TIMESTAMP_FORMAT = "%Y.%m.%d %H:%M:%S"
...
def _benchmark_batch_standalone(
    combo_batch: list[tuple[int, ...]],
    trade_matrix: np.ndarray,
    num_strategies: int,
):
    """Executes the same vectorized batch math used by optimization without ranking."""
    num_combos = len(combo_batch)
    strategy_mask = np.zeros((num_strategies, num_combos), dtype=np.float64)
    for combo_position, strategy_indices in enumerate(combo_batch):
        strategy_mask[list(strategy_indices), combo_position] = 1.0
    ...
    batch_returns = trade_matrix @ strategy_mask
    _ = compute_batch_performance(batch_returns)

    del batch_returns, strategy_mask
...
def _benchmark_worker_partition(args: tuple[int, ...]) -> tuple[int, int, int]:
    """Worker: benchmark throughput for a partition without keeping ranking state."""
    (
        trade_matrix,
        correlation_matrix,
        num_strategies,
    ...
    while True:
        valid_combos, discarded_count = _filter_combos_standalone(
            combo_gen, correlation_matrix, corr_filter_batch_size, max_corr
        )
        processed += discarded_count
        ...
        for batch_start in range(0, len(valid_combos), matrix_algebra_batch_size):
            ...
            _benchmark_batch_standalone(batch_slice, trade_matrix, num_strategies)
            batch_count = len(batch_slice)
            processed += batch_count
            valid += batch_count

    return processed, valid, discarded
...
class BruteForceEngine:
    ...
    def __init__(
        ...
        self.correlation_matrix = (
            ...
            else self._compute_correlation(correlation_period)
        )
    ...
    def benchmark(
        ...
        """Measures real optimization throughput and extrapolates it."""
        trade_matrix = self.wide_trades.drop("Horário").to_numpy().astype(np.float64)
        num_strategies = len(self.strategy_names)
        total_combos = sum(
            math.comb(num_strategies, k) for k in range(min_assets, max_assets + 1)
        ...
        if total_combos == 0:
            return BenchmarkResult(
                ...
                discarded_combinations=0,
                elapsed_seconds=0.0,
                combinations_per_second=0.0,
                estimated_combinations_in_target=0,
        ...
        logger.info(
            f"Benchmark: measuring throughput for {sample_seconds:.1f}s "
            f"| search space: {total_combos:,} combinations "
            f"| workers: {self.num_workers}"
        )

        if self.num_workers == 1:
            stats = self._benchmark_single_process(
                trade_matrix,
                num_strategies,
                min_assets,
                max_assets,
            ...
            stats = self._benchmark_multiprocess(
                trade_matrix,
                num_strategies,
                min_assets,
                max_assets,
        ...
        elapsed_seconds = max(float(stats["elapsed_seconds"]), 1e-9)
        processed_combinations = int(stats["processed_combinations"])
        combinations_per_second = processed_combinations / elapsed_seconds
        estimated_combinations_in_target = int(combinations_per_second * target_seconds)
        estimated_completion_seconds = (
            (total_combos / combinations_per_second)
        ...
        return BenchmarkResult(
            ...
            valid_combinations=int(stats["valid_combinations"]),
            discarded_combinations=int(stats["discarded_combinations"]),
            elapsed_seconds=elapsed_seconds,
            combinations_per_second=combinations_per_second,
            estimated_combinations_in_target=estimated_combinations_in_target,
            estimated_completion_seconds=estimated_completion_seconds,
            completed_search=bool(stats["completed_search"]),
            sample_seconds=sample_seconds,
            target_seconds=target_seconds,
            num_workers=self.num_workers,
        )

    def _benchmark_single_process(
        self,
        trade_matrix: np.ndarray,
        num_strategies: int,
        min_assets: int,
        ...
        """Single-process benchmark path using the real vectorized batch math."""
        processed = 0
        valid = 0
        discarded = 0
        completed_search = True
        ...
        return {
            ...
            "discarded_combinations": discarded,
    ...
    def _benchmark_multiprocess(
        ...
        """Multi-process benchmark path using small task chunks and real workers."""
        processed = 0
        valid = 0
        discarded = 0
        completed_search = True
        ...
        try:
            ...
            for num_assets in range(min_assets, max_assets + 1):
                ...
                task_chunk_size = max(self.num_workers * 4, 1)

                for chunk_start in range(0, len(all_prefixes), task_chunk_size):
                    if time.perf_counter() >= deadline:
                        completed_search = False
                        break

                    prefix_chunk = all_prefixes[
                        chunk_start : chunk_start + task_chunk_size
                    ]
                    args_list = [
                        (
                            trade_matrix,
                            ...
                            num_assets,
                            prefix,
                            self.corr_filter_batch_size,
                            self.matrix_algebra_batch_size,
                            max_corr,
                        )
                    ...
                    iterator = pool.imap_unordered(
                        _benchmark_worker_partition, args_list, chunksize=1
                    )
                    for worker_processed, worker_valid, worker_discarded in iterator:
                        processed += worker_processed
                        valid += worker_valid
                        discarded += worker_discarded
                        if time.perf_counter() >= deadline:
                            completed_search = False
                            pool.terminate()
                            pool = None
    ...
    def ensure_detailed_metrics(self) -> None:
        """Hydrates detailed MT5-style metrics only when they are still missing."""
        if not self.best_portfolios:
            return
        if "Total_Trades" in self.best_portfolios[0]:
            return
        logger.info("Calculating detailed metrics for selected portfolios...")
        self._enrich_with_detailed_metrics(self.best_portfolios)
