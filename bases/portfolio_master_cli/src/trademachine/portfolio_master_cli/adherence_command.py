...
========================
SQX vs MT5 adherence check functionality for PortfolioCLI.
"""
...
if TYPE_CHECKING:
    from trademachine.portfoliomaster.public import AdherenceResult

DEFAULT_MT5_ADHERENCE_DIR = (
    "components/portfoliomaster/test/trademachine/portfoliomaster/reports"
)
DEFAULT_SQX_ADHERENCE_DIR = (
    "components/portfoliomaster/test/trademachine/portfoliomaster/reports-sqx"
)
...
class AdherenceMixin:
    ...
    def adherence_run(
        ...
        result: AdherenceResult = cli_module.run_adherence_check(
            mt5_dir,
            sqx_dir,
            threshold,
            pearson_threshold,
        ...
        if not quiet:
            ...
            for s in result.strategies:
                insufficient_data = getattr(s, "insufficient_data", False)
                ...
                print(
                    ...
                    f"{Fore.GREEN}{s.mt5_retdd:>8.2f} {Fore.GREEN}{s.sqx_retdd:>8.2f}  {status_color}{status_str}{Style.RESET_ALL}"
                )
            print(sep)
            print(
        ...
        abs_html = cli_module.generate_adherence_report_html(
            result, report_path, open_browser
        )
        ...
        if export_passed_reports:
            ...
            for s in result.strategies:
                if (
                    not s.passes
                    or getattr(s, "insufficient_data", False)
                    or not getattr(s, "mt5_report_path", None)
                ):
                    continue
