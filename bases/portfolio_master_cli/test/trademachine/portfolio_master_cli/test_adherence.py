...
def test_run_adherence_check_mismatch(tmp_path):
    ...
    with (
        patch(
            "trademachine.portfoliomaster.services.adherence._load_mt5_strategies"
        ) as mock_mt5,
        patch(
            "trademachine.portfoliomaster.services.adherence.parse_sqx_directory"
        ) as mock_sqx,
    ):
...
def test_adherence_run_cli(tmp_path):
    """Test PortfolioCLI.adherence_run with mocked backend."""
    cli = PortfolioCLI()
    ...
    with (
        patch(
            "trademachine.portfolio_master_cli.cli.run_adherence_check",
            return_value=mock_result,
        ),
        patch(
            "trademachine.portfolio_master_cli.cli.generate_adherence_report_html"
        ) as mock_gen_html,
    ):
        cli.adherence_run(
            mt5_dir="mock_mt5",
