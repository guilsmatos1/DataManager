...
class DataHandler:
    ...
    def check_local_completeness(
        self, bar_type: BarType, req_start: pd.Timestamp, req_end: pd.Timestamp
    ):
        ...
        if first_ts > req_start or last_ts < req_end:
            print(
                f"Local incomplete: {first_ts} to {last_ts}. Needed: {req_start} to {req_end}"
            )
            return False
        return True

    def sync_server(
        self, source: str, asset: str, req_start: pd.Timestamp, req_end: pd.Timestamp
    ):
...
class MACDStrategy(Strategy):
    ...
    def on_stop(self):
        ...
        print(
            f"H1 Bars: {self.h1_received:,} | Trades: {self.trades} | MACD Init: {self.macd.initialized}"
        )
...
def main():
    ...
    strategy = MACDStrategy(
        config=MACDConfig(instrument_id=instrument.id, h1_bar_type=h1_type)
    )
