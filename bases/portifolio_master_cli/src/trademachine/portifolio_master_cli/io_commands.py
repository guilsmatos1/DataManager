...
IO Commands Module
==================
Import, export, and output functionality for PortifolioCLI.
"""
...
class IOMixin:
    ...
    def _save_best_portfolio_trades(
        ...
        for rank, portfolio in enumerate(portfolios, start=1):
            ...
            for name in combo_list:
                if name in self.portfolio_manager.strategies:  # type: ignore[attr-defined]
                    lot = self.portfolio_manager.strategy_lots.get(name, 1.0)  # type: ignore[attr-defined]
                    df = self.portfolio_manager.strategies[name].with_columns(  # type: ignore[attr-defined]
    ...
    def _import_from_parquet(self, filepath: str, plot_after_load: bool) -> None:
        ...
        for name in strategy_names:
            ...
            if has_lot_col:
                ...
                if len(lot_vals) > 0:
                    ...
                    if lot_median is not None:
                        ...
                        if self.portfolio_manager.strategy_lots.get(name) != new_lot:  # type: ignore[attr-defined]
                            self.portfolio_manager.strategy_lots[name] = new_lot  # type: ignore[attr-defined]
                            lots_updated = True
            ...
            self.portfolio_manager.strategies[name] = strat_df  # type: ignore[attr-defined]
    ...
    def _run_montecarlo(
        ...
        all_dfs = [
            self.portfolio_manager.strategies[name]  # type: ignore[attr-defined]
            for name in combo
            if name in self.portfolio_manager.strategies  # type: ignore[attr-defined]
        ]
        if not all_dfs:
