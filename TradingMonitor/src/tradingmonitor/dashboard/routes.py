...
from tradingmonitor.dashboard.serializers import (
    AccountResponse,
...
from tradingmonitor.ingestion.tcp_server import invalidate_cache
from tradingmonitor.utils.mt5_parser import _EN_TO_PT_COLUMNS, MT5ReportParser
...
def get_real_overview(db: Session = Depends(get_db)):
    ...
    np_map: dict[str, float] = dict(
        ...
        .filter(Deal.strategy_id.in_(sids), Deal.type.in_([DealType.BUY, DealType.SELL]))
    ...
    latest_eq_rows = (
        ...
        .join(subq, (EquityCurve.strategy_id == subq.c.strategy_id) & (EquityCurve.timestamp == subq.c.latest_ts))
        .all()
    )
    ...
    for row in equity_rows:
        equity_by_sid[row.strategy_id].append(
            {"ts": row.timestamp.isoformat(), "balance": float(row.balance), "equity": float(row.equity)}
        )
    ...
    for s in real_strategies:
        ...
        floating = (equity - balance) if (equity is not None and balance is not None) else 0.0
...
def list_accounts(db: Session = Depends(get_db)):
    ...
    net_profits = dict(
        db.query(Strategy.account_id, func.sum(Deal.profit + Deal.commission + Deal.swap))
...
def delete_strategy(strategy_id: str, db: Session = Depends(get_db)):
    ...
    db.execute(text("DELETE FROM equity_curve WHERE strategy_id = :sid"), {"sid": strategy_id})
    db.execute(
        text("""
    ...
    db.execute(text("DELETE FROM backtests WHERE strategy_id = :sid"), {"sid": strategy_id})
    db.execute(
        text("DELETE FROM portfolio_strategy WHERE strategy_id = :sid"), {"sid": strategy_id}
    )
    db.delete(s)
    db.commit()
...
def get_strategy_advanced_metrics(
    ...
    from tradingmonitor.metrics.repository import get_strategy_deals, get_strategy_equity_curve
    ...
    try:
        ...
        if initial_balance and "Net Profit" in metrics and metrics["Net Profit"] is not None:
            metrics["Return on Capital (%)"] = (metrics["Net Profit"] / initial_balance) * 100
...
def get_strategy_trade_stats(strategy_id: str, db: Session = Depends(get_db)):
    ...
    by_dow = [
        {"dow": i + 1, "label": DOW_LABELS[i], "count": 0, "net_profit": 0.0} for i in range(7)
    ]
    for r in dow_rows:
        d = int(r.key) - 1
        by_dow[d] = {
...
@router.get("/strategies/{strategy_id}/equity", response_model=list[EquityPointResponse])
...
def get_strategy_deals(
    ...
    total = base.count()
    deals = (
        base.order_by(Deal.timestamp.desc()).offset((page - 1) * page_size).limit(page_size).all()
    )
    ...
    for p in portfolios:
        ...
        if strategy_ids:
            np_val = (
                ...
                .filter(
                    Deal.strategy_id.in_(strategy_ids), Deal.type.in_([DealType.BUY, DealType.SELL])
                )
                .scalar()
            )
            ...
            for sid in strategy_ids:
                ...
                if pts:
                    frames.append(pd.Series({ts: eq for ts, eq in pts}, name=sid, dtype=float))
            if frames:
                ...
                r.max_drawdown = float(dd_series.max()) if not dd_series.isna().all() else None
...
def create_portfolio(payload: PortfolioCreate, db: Session = Depends(get_db)):
    ...
    if payload.strategy_ids:
        strategies = db.query(Strategy).filter(Strategy.id.in_(payload.strategy_ids)).all()
...
def update_portfolio(portfolio_id: int, payload: PortfolioUpdate, db: Session = Depends(get_db)):
    ...
    if not p:
        raise HTTPException(status_code=404, detail="Portfolio not found")
    for field, value in payload.model_dump(exclude_unset=True, exclude={"strategy_ids"}).items():
        setattr(p, field, value)
    if payload.strategy_ids is not None:
        p.strategies = db.query(Strategy).filter(Strategy.id.in_(payload.strategy_ids)).all()
    db.commit()
    db.refresh(p)
    return PortfolioResponse.from_orm_portfolio(p)
...
def get_portfolio_equity(portfolio_id: int, db: Session = Depends(get_db)):
    ...
    if not strategy_ids:
        return []
    from tradingmonitor.metrics.repository import get_strategy_equity_curve
    ...
    return [{"timestamp": ts.isoformat(), "equity": float(v)} for ts, v in combined.items()]
...
def get_portfolio_correlation(
    ...
    from tradingmonitor.metrics.calculator import calculate_correlation_matrix
...
def get_portfolio_concurrency(
    ...
    if len(strategy_ids) < 2:
        return {"error": "Need at least 2 strategies in this portfolio."}
    from tradingmonitor.metrics.calculator import calculate_concurrency
...
def get_portfolio_advanced_metrics(
    ...
    combined_equity = pd.concat(all_equity).sort_index() if all_equity else pd.DataFrame()
...
def _inject_bt_net_profit(bt: Backtest, db: Session) -> BacktestResponse:
    ...
    net = (
        db.query(func.sum(BacktestDeal.profit + BacktestDeal.commission + BacktestDeal.swap))
        .filter(
...
def get_backtest_metrics(backtest_id: int, db: Session = Depends(get_db)):
    ...
    if not bt:
        raise HTTPException(status_code=404, detail="Backtest not found")
    from tradingmonitor.metrics.calculator import calculate_metrics_from_df
    from tradingmonitor.metrics.repository import get_backtest_deals, get_backtest_equity
...
@router.get("/backtests/{backtest_id}/equity", response_model=list[BacktestEquityPointResponse])
...
def get_backtest_deals_endpoint(
    ...
    if not bt:
        raise HTTPException(status_code=404, detail="Backtest not found")
    total = db.query(BacktestDeal).filter(BacktestDeal.backtest_id == backtest_id).count()
    deals = (
...
def get_backtest_daily(backtest_id: int, db: Session = Depends(get_db)):
    ...
    rows = (
        db.query(
            ...
            func.sum(BacktestDeal.profit + BacktestDeal.commission + BacktestDeal.swap).label(
                "net_profit"
            ),
        )
        .filter(BacktestDeal.backtest_id == backtest_id)
...
def get_backtest_trade_stats(backtest_id: int, db: Session = Depends(get_db)):
    ...
    for r in hour_rows:
        h = int(r.key)
        by_hour[h] = {"hour": h, "count": int(r.count), "net_profit": round(float(r.net_profit or 0), 2)}
    ...
    for r in dow_rows:
        d = int(r.key) - 1
        by_dow[d] = {"dow": d + 1, "label": DOW_LABELS[d], "count": int(r.count), "net_profit": round(float(r.net_profit or 0), 2)}
...
def get_floating_pnl(db: Session = Depends(get_db)):
    ...
    latest_ts = (
        db.query(EquityCurve.strategy_id, func.max(EquityCurve.timestamp).label("max_ts"))
        .group_by(EquityCurve.strategy_id)
        .subquery()
    )
    rows = (
...
def health_check(db: Session = Depends(get_db)):
    ...
    from tradingmonitor.ingestion.tcp_server import get_ingestion_status, get_server_uptime_seconds
...
def ingestion_status():
    from tradingmonitor.ingestion.tcp_server import get_ingestion_status
...
def export_strategy_deals(strategy_id: str, db: Session = Depends(get_db)):
    ...
    if not strategy:
        raise HTTPException(status_code=404, detail="Strategy not found")
    query = db.query(Deal).filter(Deal.strategy_id == strategy_id).order_by(Deal.timestamp)
...
def export_portfolio_deals(portfolio_id: int, db: Session = Depends(get_db)):
    ...
    query = db.query(Deal).filter(Deal.strategy_id.in_(strategy_ids)).order_by(Deal.timestamp)
...
async def upload_backtest_html(
    ...
    for upload_file in files:
        ...
        try:
            ...
            if not strategy:
                raise ValueError(f"Estratégia com Magic Number {magic_number} não cadastrada")
            ...
            if col_balance is not None:
                balance_rows = deals_df[deals_df[col_tipo].str.strip().str.lower() == "balance"]
