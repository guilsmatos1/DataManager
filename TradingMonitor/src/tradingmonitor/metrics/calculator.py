...
def calculate_metrics_from_df(
    ...
    win_rate = (winning_trades / len(trading_deals)) * 100 if len(trading_deals) > 0 else 0
    ...
    if not equity_df.empty:
        ...
        if len(daily_equity) > 1:
            ...
            if not daily_returns.empty:
                try:
                    metrics["Max Drawdown (%)"] = qs.stats.max_drawdown(daily_returns) * 100
...
def calculate_correlation_matrix(
    ...
    if len(series) < 2:
        return {"error": "Need at least 2 strategies with deal data to compute correlation."}

    combined = pd.DataFrame(series).fillna(0)
    combined = combined.loc[(combined != 0).any(axis=1)]

    if len(combined) < 3:
        return {"error": "Not enough overlapping periods to compute correlation (need ≥ 3)."}
    ...
    for idx_a, strategy_a in enumerate(strategies):
        for idx_b in range(idx_a + 1, len(strategies)):
            ...
            if not np.isnan(corr_value):
                correlation_pairs.append((strategy_a, strategy_b, round(float(corr_value), 3)))
    ...
    avg_corr = (
        round(float(np.mean([p[2] for p in correlation_pairs])), 3) if correlation_pairs else None
    )

    return {
        "strategies": strategies,
        "matrix": [
            [None if pd.isna(v) else round(float(v), 3) for v in row] for row in corr.values
        ],
...
def calculate_concurrency(strategy_ids: list[str], since: datetime | None = None) -> dict:
...
def calculate_portfolio_metrics(strategy_ids: list[str]) -> dict:
    ...
    if all_equity:
        ...
        equity_combined_df = pd.concat([df["equity"] for df in all_equity], axis=1).sort_index()
