...
from tradingmonitor.db.database import engine
...
def get_strategy_equity_curve(strategy_id: str) -> pd.DataFrame:
    ...
    try:
        query = text("SELECT * FROM equity_curve WHERE strategy_id = :sid ORDER BY timestamp")
        return pd.read_sql(query, engine, params={"sid": strategy_id}, index_col="timestamp")
    except Exception as e:
...
def get_backtest_deals(backtest_id: int) -> pd.DataFrame:
    ...
    try:
        query = text("SELECT * FROM backtest_deals WHERE backtest_id = :bid ORDER BY timestamp")
        return pd.read_sql(query, engine, params={"bid": backtest_id}, index_col="timestamp")
    except Exception as e:
...
def get_backtest_equity(backtest_id: int) -> pd.DataFrame:
    ...
    try:
        query = text("SELECT * FROM backtest_equity WHERE backtest_id = :bid ORDER BY timestamp")
        return pd.read_sql(query, engine, params={"bid": backtest_id}, index_col="timestamp")
    except Exception as e:
