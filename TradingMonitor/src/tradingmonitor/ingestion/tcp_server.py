...
HEARTBEAT_FILE = "/tmp/tradingmonitor_heartbeat"
...
_server_start_time: datetime | None = None  # set to non-None only when start_server() is called
...
_DEAD_LETTER_FILE = "/tmp/tradingmonitor_dead_letters.jsonl"
...
def ensure_strategy_exists(db, strategy_id: str, symbol: str = None, account_id: str | None = None):
...
def _save_dead_letter(db, topic: str, raw: str, error: str):
    try:
        err = IngestionError(topic=topic, raw_message=raw[:4096], error_message=error[:2048])
        db.add(err)
        db.commit()
    except Exception as e:
        db.rollback()
            ...
            while "\n" in buf:
                ...
                try:
                    ...
                    if topic == "DEAL":
                        ...
                        _link_strategies_to_account(db, conn_strategies_seen, conn_account_id)
...
def process_account(db, data: AccountSchema):
    ...
    if data.free_margin < (data.balance * (settings.margin_threshold_pct / 100.0)):
        notifier.notify_low_margin(acc_id, data.free_margin, settings.margin_threshold_pct)
...
def process_backtest_deal(db, data: BacktestDealSchema):
    ...
    if bt_id is None:
        raise ValueError(f"Backtest not found for strategy={magic} run_id={data.run_id}")
    stmt = (
...
def process_backtest_end(db, data: BacktestEndSchema):
    ...
    bt_id = _get_or_lookup_backtest_id(db, magic, data.run_id)
    if bt_id is None:
        logger.warning("BACKTEST_END: backtest not found strategy=%s run_id=%s", magic, data.run_id)
        return
