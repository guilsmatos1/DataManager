...
class _TimestampMixin(BaseModel):
    ...
    @field_validator("time", "start_date", "end_date", mode="before", check_fields=False)
    @classmethod
...
class _TradingFieldsMixin(BaseModel):
    ...
    def _validate_trading_fields(self) -> "_TradingFieldsMixin":
        if self.type != "balance":
            if self.volume <= 0:
                raise ValueError(f"volume must be > 0 for trading deals, got {self.volume}")
            if self.price < 0:
                raise ValueError(f"price cannot be negative, got {self.price}")
        return self


class DealSchema(_TimestampMixin, _TradingFieldsMixin):
    time: int
    ticket: int
    magic: int
    symbol: str
...
class EquitySchema(_TimestampMixin):
    time: int
    magic: int
    balance: float
    equity: float
