...
class Account(Base):
    ...
    id: Mapped[str] = mapped_column(String, primary_key=True)  # Account Number (MT5 Login)
...
class Deal(Base):
    ...
    __table_args__ = (
        UniqueConstraint("ticket", "strategy_id", "timestamp", name="uq_deal_ticket_strategy"),
...
class EquityCurve(Base):
    ...
    __table_args__ = (Index("ix_equity_strategy_timestamp", "strategy_id", "timestamp"),)
...
class Backtest(Base):
    ...
    __table_args__ = (
        UniqueConstraint("strategy_id", "client_run_id", name="_backtest_strategy_run_uc"),
    )
