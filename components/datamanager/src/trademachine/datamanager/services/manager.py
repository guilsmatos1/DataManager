...
class DataManager:
    ...
    def download_data(
        ...
        Args:
            pbar: Optional tqdm-compatible progress object. When None, a local
                  tqdm instance is created for CLI output.
        """
        ...
        # Use provided pbar (API context) or create a local tqdm (CLI context)
        own_pbar = pbar is None
        if own_pbar:
            pbar = tqdm(total=total_days, desc=f"Downloading {asset}", unit="days")
    ...
    def update_data(
        ...
        Args:
            pbar: Optional tqdm-compatible progress object for API progress reporting.
        """
        target_timeframe = timeframe.upper()
        ...
        if last_date.date() >= now.date() and (now - last_date).total_seconds() < 3600:
            ...
            if pbar is not None:
                pbar.total = 1  # type: ignore[union-attr]
                pbar.update(1)  # type: ignore[union-attr]
        else:
            logger.info(f"Updating {asset} M1 from {last_date}...")
            fetch_days = max((now - last_date).days, 1)
            # total = fetch days + 1 (append) + 1 (resample/finalize)
            if pbar is not None:
                pbar.total = fetch_days + 2  # type: ignore[union-attr]

            fetcher = self._get_fetcher(source)
            new_df = fetcher.fetch_data(asset, last_date, now, pbar=pbar)
