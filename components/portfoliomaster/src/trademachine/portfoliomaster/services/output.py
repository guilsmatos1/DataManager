...
class OptimizationOutputService:
    ...
    def build_rank_payload(
        ...
        for rank, portfolio in enumerate(portfolios, start=1):
            ...
            lots = {name: round(self.strategy_lots.get(name, 1.0), 2) for name in combo}
            weights = self.compute_weights(combo)
            paired_lots = {name: round(lots[name] * weights[name], 2) for name in combo}
            serialized.append(
                {
                    "rank": rank,
    ...
    def save_rank_json(self, output_dir: str, payload: dict[str, Any]) -> str:
