            ...
            .with_columns(
                [
                    ...
                    pl.col("Horário").str.to_datetime(
                        MT5_TIMESTAMP_FORMAT, strict=False
                    ),
                ]
            )
            .with_columns(
...
# ---------------------------------------------------------------------------
# Standalone trade-dataset filter functions
# ---------------------------------------------------------------------------
...
def filter_trades_by_date(
    ...
    """Filters a trades DataFrame to the given date range (inclusive).

    Args:
        trades: Long-format trades DataFrame with a ``Horário`` datetime column.
        date_initial: Start date in ``YYYY-MM-DD`` format, or ``None`` for no lower bound.
        date_final:   End date in ``YYYY-MM-DD`` format, or ``None`` for no upper bound.

    Returns:
        Filtered DataFrame.

    Raises:
        ValidationError: If the resulting DataFrame is empty.
    """
    from datetime import datetime as _dt

    from trademachine.portfoliomaster.core.exceptions import ValidationError

    mask = pl.lit(True)
    if date_initial:
        mask = mask & (pl.col("Horário") >= _dt.strptime(date_initial, "%Y-%m-%d"))
    if date_final:
        end_dt = _dt.strptime(date_final, "%Y-%m-%d").replace(
...
def filter_trades_by_strategy(
    ...
    """Filters a trades DataFrame to the requested strategy names.

    Args:
        trades:          Long-format trades DataFrame with a ``Strategy`` column.
        requested_names: Names of strategies to include.
        loaded_names:    Names of all strategies currently in memory (used for validation).
        min_assets:      Minimum number of valid strategies required.

    Returns:
        Filtered DataFrame.

    Raises:
        ValidationError: If fewer valid strategies are found than ``min_assets`` requires.
    """
    from trademachine.portfoliomaster.core.exceptions import ValidationError

    valid_names = [n for n in requested_names if n in loaded_names]
    missing = [n for n in requested_names if n not in loaded_names]
    if missing:
    ...
    return trades.filter(pl.col("Strategy").is_in(valid_names))


def exclude_strategies_from_trades(
    trades: pl.DataFrame,
    excluded_names: list[str],
    loaded_names: list[str],
    min_assets: int,
) -> pl.DataFrame:
    """Removes excluded strategies from a trades DataFrame.

    Args:
        trades:         Long-format trades DataFrame with a ``Strategy`` column.
        excluded_names: Names of strategies to remove.
        loaded_names:   Names of all strategies currently in memory (used for validation).
        min_assets:     Minimum number of remaining strategies required.

    Returns:
        Filtered DataFrame.

    Raises:
        ValidationError: If fewer strategies remain than ``min_assets`` requires.
    """
    from trademachine.portfoliomaster.core.exceptions import ValidationError

    valid_exclusions = [n for n in excluded_names if n in loaded_names]
    missing = [n for n in excluded_names if n not in loaded_names]
    if missing:
        logger.warning(
            f"Strategies not found for exclusion (check spelling): {', '.join(missing)}"
        )

    remaining_names = [n for n in loaded_names if n not in valid_exclusions]
    if len(remaining_names) < min_assets:
        raise ValidationError(
            f"Only {len(remaining_names)} strategies remain after exclusion, "
            f"but {min_assets} required."
        )
    return trades.filter(~pl.col("Strategy").is_in(valid_exclusions))
