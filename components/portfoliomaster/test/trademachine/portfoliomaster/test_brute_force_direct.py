...
def _run_cmd(extra_args, output_dir, cwd=None):
    base = [
        sys.executable,
        "-m",
        "trademachine.portfolio_master_cli.main",
        "optimize",
        "--load",
        REPORTS_DIR,
        "--workers",
...
def test_single_strategy_engine_matches_direct_calculation(tmp_path, pt_parser):
    """
    ...
    assert len(names) >= 2, "Need at least 2 parsed reports"
    target_name = names[0]
    second_name = names[1]

    direct_metrics = calculate_metrics_from_deals(
        pt_parser.deals_by_expert[target_name]
    )
