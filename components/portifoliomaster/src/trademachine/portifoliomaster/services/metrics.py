...
def compute_streak_statistics(trades_df: pd.DataFrame) -> dict[str, float]:
    ...
    return {
        "avg_win_streak": round(
            sum(win_streaks) / len(win_streaks) if win_streaks else 0.0, 1
        ),
        "avg_loss_streak": round(
            sum(loss_streaks) / len(loss_streaks) if loss_streaks else 0.0, 1
        ),
    }
...
def calculate_metrics_from_deals(deals_df: pd.DataFrame) -> dict[str, Any]:
    ...
    drawdowns = compute_drawdown_metrics(
        processed_df["Saldo"].tolist(), initial_balance
    )
