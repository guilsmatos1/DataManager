...
from trademachine.portifoliomaster.utils.sqx_parser import (
    parse_sqx_csv,
    parse_sqx_directory,
)
...
def test_run_adherence_check_mismatch(tmp_path):
    ...
    with (
        patch(
            "trademachine.portifoliomaster.services.adherence._load_mt5_strategies"
        ) as mock_mt5,
        patch(
            "trademachine.portifoliomaster.services.adherence.parse_sqx_directory"
        ) as mock_sqx,
    ):
...
def test_adherence_run_cli(tmp_path):
    ...
    with (
        patch(
            "trademachine.portifolio_master_cli.cli.run_adherence_check",
            return_value=mock_result,
        ),
        patch(
            "trademachine.portifolio_master_cli.cli.generate_adherence_report_html"
        ) as mock_gen_html,
    ):
        cli.adherence_run(
            mt5_dir="mock_mt5",
