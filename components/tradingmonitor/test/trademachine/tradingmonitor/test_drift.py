...
import pandas as pd
from trademachine.tradingmonitor.analysis.drift import (
    DriftReport,
    _compute_var,
    _notify_drift,
    check_performance_drift,
)
from trademachine.tradingmonitor.db.models import Backtest
...
class TestCheckPerformanceDrift:
    ...
    def test_returns_none_when_no_live_data(self):
        ...
        with patch(
            "trademachine.tradingmonitor.analysis.drift.SessionLocal",
            return_value=mock_db,
        ):
            with patch(
                "trademachine.tradingmonitor.analysis.drift.get_strategy_deals",
                return_value=pd.DataFrame(),  # Empty deals
            ):
                with patch(
                    "trademachine.tradingmonitor.analysis.drift.get_strategy_equity_curve",
    ...
    def test_ignores_backtest_when_insufficient_trades(self):
        ...
        with patch(
            "trademachine.tradingmonitor.analysis.drift.settings"
        ) as mock_settings:
            ...
            with patch(
                ...
                with patch(
                    ...
                    with patch(
                        ...
                        with patch(
                            "trademachine.tradingmonitor.analysis.drift._notify_drift"
                        ):
            ...
            with patch(
                ...
                with patch(
                    ...
                    with patch(
                        ...
                        with patch(
                            "trademachine.tradingmonitor.analysis.drift.get_backtest_deals",
                            return_value=deals,
                        ):
                            with patch(
                                "trademachine.tradingmonitor.analysis.drift.get_backtest_equity",
                                return_value=equity,
                            ):
                                with patch(
                                    "trademachine.tradingmonitor.analysis.drift.calculate_metrics_from_df",
    ...
    def test_detects_drawdown_breach(self):
        ...
        assert result is not None
        assert result.is_drifting is True
        assert any("Max Drawdown breach" in r for r in result.reasons)
        mock_notify.assert_called_once_with(result)
        mock_db.close.assert_called_once()
    ...
    def test_returns_none_on_exception(self):
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    "trademachine.tradingmonitor.analysis.drift.logger"
                ) as mock_logger:
...
class TestNotifyDrift:
    ...
    def test_notify_drift_calls_notifier(self):
        ...
        with patch(
            "trademachine.tradingmonitor.analysis.drift.notifier"
        ) as mock_notifier:
...
class TestDependencyInjection:
    ...
    def test_drift_detection_with_custom_threshold_via_di(self):
        ...
        """
        from trademachine.tradingmonitor.analysis.drift import check_performance_drift
