...
from portifoliomaster.api.cli import PortifolioCLI, _flag_value
from portifoliomaster.core.config import AppConfig
from portifoliomaster.core.exceptions import ValidationError
from portifoliomaster.utils.logger import LOGGER_NAME, configure_console_streams, setup_logger
...
def _handle_subcommand(
    ...
    if cmd == "cache":
        ...
        if sub == "info":
            ...
            if not directory:
                print("Usage: portifoliomaster cache rebuild <directory> [--cache-path PATH]")
                sys.exit(1)
        ...
        if sub == "correlations":
            ...
            cli_app.inspect_correlations(period=period, top_n=top_n, threshold=threshold)
        elif sub == "strategy":
        ...
        if sub == "pairing":
            ...
            apply = "--apply" in args
            cli_app.load_reports()
            cli_app.drawdown_pairing(target_dd=target_dd, lot_tick=lot_tick, apply=apply)
        else:
        ...
        threshold = float(threshold_raw) if threshold_raw else config.adherence_threshold
        pearson_threshold = float(pearson_raw) if pearson_raw else config.adherence_pearson
...
def validate_cli_args(args):
    ...
    if args.min < 1:
        raise ValidationError(f"Minimum assets (--min) must be at least 1. Got: {args.min}")
...
def main():
    ...
    parser.add_argument("--optimize", action="store_true", help="Run optimization process")
    parser.add_argument("--strats", type=str, help="Filter strategies (comma-separated names)")
    ...
    parser.add_argument(
        "--corr-period", type=str, choices=["H", "D", "W", "M"], default=config.corr_period
    )
    parser.add_argument("--rank", type=str, choices=["RetDD", "NetProfit"], default=config.rank_by)
    parser.add_argument(
        "--workers",
        type=int,
    ...
    parser.add_argument(
        "--print", action="store_true", default=config.print_results, help="Print results table"
    )
    parser.add_argument(
        "--output",
        nargs="?",
    ...
    parser.add_argument("--import-p", type=str, help="Import and analyze saved portfolio JSON")
    parser.add_argument(
        "--quiet",
        action="store_true",
