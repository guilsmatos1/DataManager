...
from portifoliomaster.core.exceptions import (
    DataError,
    OptimizationError,
    ParserError,
    ValidationError,
...
from portifoliomaster.services.optimization import BruteForceEngine
...
class PortifolioCLI:
    ...
    def load_reports(self, directory_path: str | None = None, use_cache: bool = True):
        ...
        if use_cache and os.path.exists(cache_filepath):
            logger.info(f"Loading cached strategies from: {os.path.abspath(cache_filepath)}")
            try:
                ...
                self.loaded_expert_names = [col for col in cached_df.columns if col != "Horário"]
                ...
                logger.info(f"[OK] {len(self.loaded_expert_names)} strategies loaded from cache.")
                return
            except Exception as e:
                logger.warning(f"Cache invalid or incompatible: {e}.")

        if not directory_path:
            logger.error("No cache found and no directory provided to scan for reports.")
            return
        ...
        for filepath in tqdm(report_files, desc="Parsing"):
            try:
                ...
                tqdm.write(f"   - Unexpected error parsing {os.path.basename(filepath)}: {e}")
        ...
        logger.info(f"Total: {len(self.loaded_expert_names)} strategies successfully loaded.")
    ...
    def _apply_strategy_filter(
        ...
        if missing:
            logger.warning(f"Strategies not found (check spelling): {', '.join(missing)}")
    ...
    def _save_best_portfolio_trades(self, portfolios: list[dict], file_path: str) -> int:
        ...
        for rank, portfolio in enumerate(portfolios, start=1):
            ...
            combo_list = (
                [s.strip() for s in combo.split(",")] if isinstance(combo, str) else list(combo)
            )
    ...
    def _compute_weights(self, combo) -> dict[str, float]:
        ...
        for name in strats:
            if name in self.portfolio_manager.strategies:
                returns = self.portfolio_manager.strategies[name]["Net_Profit"].to_numpy()
        ...
        total = sum(inv)
        return {name: round(iv / total, 4) for name, iv in zip(strats, inv, strict=True)}
    ...
    def _save_output_dir(self, engine, kwargs: dict) -> str:
        ...
        for i, p in enumerate(engine.best_portfolios):
            ...
            entry = {
                ...
                "lots": {
                    name: self.portfolio_manager.strategy_lots.get(name, 1.0) for name in combo
                },
    ...
    def _import_from_parquet(self, filepath: str, plot_after_load: bool) -> None:
        ...
        if "Strategy" not in df.columns or "Net_Profit" not in df.columns:
            logger.error("Parquet file is missing required columns (Strategy, Net_Profit).")
            return
        ...
        for name in strategy_names:
            ...
            if has_lot_col:
                lot_vals = df.filter(pl.col("Strategy") == name).select("Lot").drop_nulls()
                if len(lot_vals) > 0:
        ...
        if added:
            logger.info(f"[import] {len(added)} new strategies added: {', '.join(added)}")
            try:
                self.portfolio_manager.get_returns_matrix().write_parquet(cache_filepath)
        ...
        if plot_after_load:
            plot_portfolio_equity(tuple(strategy_names), self.portfolio_manager.strategies)
    ...
    def _import_from_json(self, filepath: str, plot_after_load: bool) -> None:
        ...
        missing = [name for name in strategy_list if name not in self.portfolio_manager.strategies]
        if missing:
            logger.error(f"Missing strategies in memory: {missing}")
            return
        logger.info(f"Imported portfolio: {label} ({len(strategy_list)} strategies)")
        if plot_after_load:
            plot_portfolio_equity(tuple(strategy_list), self.portfolio_manager.strategies)
    ...
    def _run_montecarlo(
        ...
        """
        from portifoliomaster.services.montecarlo import run_montecarlo
    ...
    def cache_info(self, cache_path: str | None = None) -> None:
        ...
        modified = _time.strftime("%Y-%m-%d %H:%M", _time.localtime(os.path.getmtime(path)))
    ...
    def config_show(self, config_path: str = "config.json") -> None:
        ...
        from portifoliomaster.core.config import AppConfig
    ...
    def drawdown_pairing(
        ...
        from portifoliomaster.services.metrics import compute_vector_metrics
        ...
        if applied == 0:
            print("[OK] No changes applied — all strategies already at target drawdown.")
            return
    ...
    def inspect_correlations(
        ...
        if n <= 12:
            ...
            header = " " * (name_w + 2) + "  ".join(s[:col_w].rjust(col_w) for s in strategy_names)
            print(header)
            print("─" * len(header))
    ...
    def inspect_strategy(self, name: str, show_chart: bool = False) -> None:
        ...
        if name not in self.portfolio_manager.strategies:
            ...
            candidates = [n for n in self.loaded_expert_names if name_lower in n.lower()]
            if not candidates:
...
@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
        ...
        while True:
            try:
                ...
                if user_input.lower() in ("help", "-h", "--help"):
                    from portifoliomaster.utils.help import get_detailed_help
    ...
    def _dispatch_subcommand(self, cmd: str, rest: list[str]) -> None:
        ...
        if cmd == "cache":
            ...
            if sub == "correlations":
                ...
                self.inspect_correlations(period=period, top_n=top_n, threshold=threshold)
            elif sub == "strategy":
            ...
            if sub == "pairing":
                ...
                apply = "--apply" in args
                self.drawdown_pairing(target_dd=target_dd, lot_tick=lot_tick, apply=apply)
            else:
    ...
    def _build_shell_parser(self) -> argparse.ArgumentParser:
        ...
        shell_parser.add_argument("--output", nargs="?", const="", default="", metavar="DIR")
