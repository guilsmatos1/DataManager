...
from portifoliomaster.core.exceptions import ParserError
...
class MT5ReportParser:
    ...
    def read_html_report(self, filepath: str) -> BeautifulSoup:
        ...
        for encoding in encodings:
            try:
                ...
                logger.debug(f"Failed to decode '{filepath}' as {encoding}, trying next.")
        raise ValueError(
    ...
    def _find_header_node(self, soup: BeautifulSoup, header_name: str):
        ...
        return soup.find(
            lambda tag: tag.name in ("th", "td") and tag.get_text(strip=True) == header_name
        )
    ...
    def extract_table_by_header(self, soup: BeautifulSoup, header_name: str) -> pd.DataFrame:
    ...
    def parse_report(self, filepath: str) -> str:
        ...
        if deals_df.empty:
            raise ParserError(f"No deals table found in '{filepath}' (tried PT and EN headers).")
