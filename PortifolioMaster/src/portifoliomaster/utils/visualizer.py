...
def plot_portfolio_equity(combo: tuple[str, ...], strategies: dict[str, pl.DataFrame]) -> None:
...
def generate_portfolio_report_html(
    ...
    columns = [
        ...
        ("Avg Consec. Losses", lambda p, i: f"{p.get('Avg_Consecutive_Losses', 0):.1f}"),
    ]
...
def generate_adherence_report_html(
    ...
    for i, s in enumerate(result.strategies):
        ...
        pearson_color = "#2ecc71" if s.pearson >= result.pearson_threshold else "#e74c3c"
