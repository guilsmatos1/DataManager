...
def _generate_combos_for_prefix(num_strategies: int, num_assets: int, prefix: tuple):
    ...
    if remaining == 0:
        yield prefix
        return
    for rest in itertools.combinations(range(prefix[-1] + 1, num_strategies), remaining):
        yield prefix + rest
...
def _filter_combos_standalone(
    ...
    while len(valid_combos) < batch_size:
        ...
        filtered_chunk = [chunk[idx] for idx, is_valid in enumerate(passes_filter) if is_valid]
    ...
    for combo_idx in range(num_combos):
        ...
        if len(top_heap) < top_n or metric_value > top_heap[0][0]:
            result = {
                ...
                "Maximum_Drawdown": float(metrics_matrix[combo_idx, METRIC_IDX_MAX_DRAWDOWN]),
            }
...
def _worker_process_partition(args: tuple) -> tuple[list[tuple[float, dict]], int]:
    ...
    while True:
        ...
        for batch_start in range(0, len(valid_combos), MATRIX_ALGEBRA_BATCH_SIZE):
            batch_slice = valid_combos[batch_start : batch_start + MATRIX_ALGEBRA_BATCH_SIZE]
    ...
    heap_items = [(item[0], item[2]) for item in local_heap]
...
class BruteForceEngine:
    ...
    def __init__(
        ...
        self.wide_trades = (
            self.all_trades_raw.pivot(
                index="Horário", on="Strategy", values="Net_Profit", aggregate_function="sum"
            )
            .fill_null(0.0)
            .sort("Horário")
            .select(["Horário"] + self.strategy_names)  # enforce alphabetical column order
        )
    ...
    def run(
        ...
        total_combos = sum(math.comb(num_strategies, k) for k in range(min_assets, max_assets + 1))
        sort_metric_index = METRIC_IDX_RET_DD if rank_by == "RetDD" else METRIC_IDX_NET_PROFIT
    ...
    def _run_single_process_loop(
        ...
        for num_assets in range(min_assets, max_assets + 1):
            ...
            while True:
                ...
                for batch_start in range(0, len(valid_combos), self.matrix_algebra_batch_size):
    ...
    def _run_multiprocess_loop(
        ...
        with multiprocessing.Pool(processes=self.num_workers) as pool:
            for num_assets in range(min_assets, max_assets + 1):
                ...
                prefix_len = _choose_prefix_len(num_strategies, num_assets, self.num_workers)
                all_prefixes = list(_enumerate_prefixes(num_strategies, num_assets, prefix_len))
                logger.info(
    ...
    def _build_portfolio_deals_df(self, combo: tuple[str, ...]) -> pd.DataFrame:
        ...
        portfolio_trades = self.all_trades_raw.filter(pl.col("Strategy").is_in(combo)).sort(
            "Horário"
        )

        if not portfolio_trades.is_empty():
            first_timestamp = portfolio_trades["Horário"][0].strftime(MT5_TIMESTAMP_FORMAT)
        else:
        ...
        if not portfolio_trades.is_empty():
            ...
            timestamps = portfolio_trades["Horário"].dt.strftime(MT5_TIMESTAMP_FORMAT).to_list()
    ...
    def run_greedy(
        ...
        while len(available) > 0:
            ...
            candidate_returns = base_returns[:, None] + trade_matrix[:, valid_candidates]
    ...
    def print_results(self, columns: list[str] | None = None):
        ...
        header = (
            f"{'Rank':<4} | {'Combo':<45} | {'# of Trades':>12} | {'Net Profit':>12} | {'RetDD':>8}"
        )
        print("=" * len(header))
        print(header)
        print("-" * len(header))
    ...
    def save_to_csv(self, filepath: str, columns: list[str] | None = None):
        ...
        df = df.with_columns(
            pl.col("Combo").map_elements(lambda x: ", ".join(x), return_dtype=pl.String),
