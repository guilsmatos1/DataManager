...
class PortfolioManager:
    ...
    def add_strategy(self, name: str, strategy_deals_df: pd.DataFrame) -> bool:
        ...
        processed_df = (
            ...
            .with_columns(
                [
                    ...
                    pl.col("Horário").str.to_datetime("%Y.%m.%d %H:%M:%S", strict=False),
                ]
            )
            .with_columns(
                [(pl.col("Lucro") + pl.col("Comissão") + pl.col("Swap")).alias("Net_Profit")]
            )
            .sort("Horário")
        )
        ...
        if "Volume" in processed_df.columns:
            try:
                ...
                logger.debug(f"Could not extract lot for '{name}'; will default to 1.0.")
    ...
    def compute_periodic_correlation(
        ...
        periodic = (
            long_df.with_columns(pl.col("Horário").dt.truncate(truncate_at).alias("_period"))
            .group_by(["_period", "Strategy"])
            .agg(pl.col("Net_Profit").sum())
            .pivot(index="_period", on="Strategy", values="Net_Profit", aggregate_function="sum")
            .sort("_period")
