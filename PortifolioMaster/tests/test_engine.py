...
def test_engine_run_with_real_reports():
    """BruteForceEngine produces valid results using real report data."""
    report_files = sorted(glob.glob(os.path.join(REPORTS_DIR, "*.html")))[:6]
    assert len(report_files) >= 2, "Need at least 2 reports for combination test"
...
def test_engine_correlation_filter_reduces_results():
    """A tight correlation filter (0.0) returns fewer results than no filter (1.0)."""
    report_files = sorted(glob.glob(os.path.join(REPORTS_DIR, "*.html")))[:8]
    assert len(report_files) >= 3, "Need at least 3 reports"
